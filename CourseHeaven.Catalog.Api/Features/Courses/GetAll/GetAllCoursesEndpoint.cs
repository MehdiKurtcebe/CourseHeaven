...
namespace CourseHeaven.Catalog.Api.Features.Courses.GetAll;

public record GetAllCoursesQuery : IRequestByServiceResult<List<CourseDto>>;

public class GetAllCoursesHandler(AppDbContext context, IMapper mapper)
    : IRequestHandler<GetAllCoursesQuery, ServiceResult<List<CourseDto>>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var course in courses) course.Category = categories.First(c => c.Id == course.CategoryId);
...
public static class GetAllCoursesEndpoint
{
    ...
    {
        group.MapGet("/",
                async (IMediator mediator) => (await mediator.Send(new GetAllCoursesQuery())).ToGenericResult())
