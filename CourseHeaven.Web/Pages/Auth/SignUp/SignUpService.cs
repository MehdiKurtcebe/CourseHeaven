...
public record KeyCloakErrorResponse(string Error);
        ...
        if (!response.IsSuccessStatusCode)
        {
            if (response.StatusCode != HttpStatusCode.InternalServerError)
            {
                var keyCloakErrorResponse =
                    await response.Content.ReadFromJsonAsync<KeyCloakErrorResponse>(cancellationToken);
                return ServiceResult.Error(keyCloakErrorResponse!.Error);
            }
